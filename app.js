...
function applyResponsiveMode() {
  ...
  const tablet = width > 720 && width <= 1024;
  root.classList.toggle('is-phone', phone);
  root.classList.toggle('is-tablet', tablet);
  root.dataset.device = phone ? 'phone' : tablet ? 'tablet' : 'desktop';
}

applyResponsiveMode();
window.addEventListener('resize', applyResponsiveMode, { passive: true });
window.addEventListener('orientationchange', applyResponsiveMode, { passive: true });
window.visualViewport?.addEventListener('resize', applyResponsiveMode, { passive: true });
